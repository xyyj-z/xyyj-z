<div align="center">

## 클라우드 엔지니어, 조영재입니다.

</div>

```python
{
    "Name": "YoungJae Cho",
    "School": "Gwangju Software Meister High School",
    "Tech": ["Public Cloud"],
    "Awards": [
        "2026 WorldSkills Korea The 61st Gwangju Regional Skills Competition - Cloud Computing (Encouragement Award)"
    ],
    "Certificate": [
        "AWS Certified Solutions Architect – Associate",
        "정보처리기능사"
    ],
    "Velog": "https://velog.io/@youngjae_/series",
    "LinkedIn": "https://www.linkedin.com/in/%EC%98%81%EC%9E%AC-%EC%A1%B0-108593387/"
}
```

<div align="center">

<br/>

<a href="https://www.credly.com/badges/08efe8b7-fc7e-42b1-b623-d2ab0bcc312d/public_url">
  <img src="https://images.credly.com/size/220x220/images/0e284c3f-5164-4b21-8660-0d84737941bc/image.png" width="150" />
</a>

<br/><br/>

[![Skills](https://skillicons.dev/icons?i=kubernetes,docker,terraform)](https://skillicons.dev)

</div>
